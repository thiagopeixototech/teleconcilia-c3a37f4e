...
import { Loader2, Plus, Pencil, Trash2, Upload, AlertCircle, ArrowRight, FileSpreadsheet } from 'lucide-react';
import { toast } from 'sonner';

const CAMPOS_ESTORNO = [
  { key: 'valor_estornado', label: 'Valor do Estorno', required: true },
  { key: 'referencia_desconto', label: 'Referência de Desconto', required: true },
...
export function MapeamentoEstornosManager() {
  const [mapeamentos, setMapeamentos] = useState<MapeamentoEstorno[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedMapeamento, setSelectedMapeamento] = useState<MapeamentoEstorno | null>(null);
  ...
  const fetchMapeamentos = async () => {
    try {
      const { data, error } = await supabase
        .from('mapeamento_estornos' as any)
        .select('*')
        .order('created_at', { ascending: false });
      if (error) throw error;
      setMapeamentos((data || []) as unknown as MapeamentoEstorno[]);
    } catch {
      toast.error('Erro ao carregar modelos de estornos');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (m: MapeamentoEstorno) => {
    setIsEditing(true);
  ...
  const handleSave = async () => {
    ...
    const requiredMissing = CAMPOS_ESTORNO.filter(c => c.required && !formMapeamento[c.key]);
    ...
    if (!matchFields.some(f => !!formMapeamento[f])) {
      setFormError('Mapeie pelo menos um campo de cruzamento (ID Make, Protocolo, CPF ou Telefone)');
      return;
    }
    setIsSaving(true);
    try {
      ...
      if (isEditing && selectedMapeamento) {
        const { error } = await supabase.from('mapeamento_estornos' as any).update(payload).eq('id', selectedMapeamento.id);
        if (error) throw error;
        toast.success('Modelo atualizado');
      } else {
        const { error } = await supabase.from('mapeamento_estornos' as any).insert([payload]);
        if (error) throw error;
  ...
  const handleDelete = async (m: MapeamentoEstorno) => {
    if (!confirm(`Excluir "${m.nome}"?`)) return;
    const { error } = await supabase.from('mapeamento_estornos' as any).delete().eq('id', m.id);
    ...
    <div className="space-y-4">
      ...
      {mapeamentos.length === 0 ? (
        <Card><CardContent className="py-8 text-center text-muted-foreground">
          ...
          <p>Nenhum modelo configurado para estornos</p>
          <p className="text-sm mt-1">Crie um modelo para facilitar a importação</p>
        </CardContent></Card>
      ) : (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{isEditing ? 'Editar Modelo' : 'Novo Modelo de Estornos'}</DialogTitle>
            <DialogDescription>Mapeie as colunas do CSV para os campos de estorno</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Ex: Relatório Desconto Vivo" value={formNome} onChange={e => setFormNome(e.target.value)} />
            </div>
            {!isEditing && (
              <div className="space-y-2">
                <Label>Arquivo CSV de Exemplo</Label>
                <div className="border-2 border-dashed rounded-lg p-4 text-center">
                  <input type="file" accept=".csv" onChange={handleFilePreview} className="hidden" id="csv-estornos-preview" />
                  <label htmlFor="csv-estornos-preview" className="cursor-pointer">
