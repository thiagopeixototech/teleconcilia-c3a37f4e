CREATE OR REPLACE FUNCTION public.get_performance_consultores(_data_inicio date, _data_fim date, _campo_data text DEFAULT 'data_venda')
 RETURNS TABLE(usuario_id uuid, consultor_nome text, total_vendas bigint, vendas_instaladas bigint, vendas_conciliadas bigint, receita_conciliada numeric, taxa_conciliacao numeric, ticket_medio numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WHERE 
    CASE 
      WHEN _campo_data = 'data_instalacao' THEN vi.data_instalacao::date
      ELSE vi.data_venda
    END BETWEEN _data_inicio AND _data_fim
    AND (
